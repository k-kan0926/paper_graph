# model_exp_fit_corr_all.py
# ---------------------------------------------------------
# θ(p)の実験値に合わせて、L(p)側の補正項を4モデルすべて同時に同定・可視化。
# - 'power', 'exp_sat', 'logistic', 'sat_power' を一括フィット
# - 「旧補正 -0.009 L0 sqrt(p)」を 置換/追加 の両モードで比較可能
# - SciPy が無い環境でも3段階グリッド探索へ自動フォールバック
...
R_m = 0.1655
r_m = 0.05094
L_off_m = 0.039338   # 幾何フィットで得たオフセット
...
# ===== 可視化設定 =====
# - "replace": 旧補正を「置換」して新補正のみを適用
# - "add"    : 旧補正に「追加」して新補正も加える（= 旧補正 + 新補正）
PLOT_MODES = ["replace", "add"]  # 片方だけ見たい場合は ["replace"] などに
...
# ---- L(p) 全体（補正の置換/追加を切替）----
def L_model(p_bar, L0_cm, params, corr_fn, replace: bool):
    base = L_base_core(p_bar, L0_cm) if replace else L_base_with_oldcorr(p_bar, L0_cm)
    return base + corr_fn(p_bar, L0_cm, params)
...
rmse0 = float(np.sqrt(np.mean((theta_base - theta_meas)**2)))
mae0  = float(np.mean(np.abs(theta_base - theta_meas)))
bias0 = float(np.mean(theta_base - theta_meas))
...
def fit_model(model_name: str, mode: str):
    """mode: 'replace' or 'add'"""
    spec = MODEL_SPECS[model_name]
    corr_fn = spec["corr"]
    x0, lb, ub = spec["x0"], spec["lb"], spec["ub"]
    replace_flag = (mode == "replace")

    def residual(x):
        L_cm = L_model(p_meas, L0_cm, x, corr_fn, replace=replace_flag)
        th   = theta_from_L_cm(L_cm, R_m, r_m, L_off_m)
        return th - theta_meas

    if use_scipy:
        sol = least_squares(residual, x0, bounds=(lb, ub), loss='soft_l1', f_scale=0.05)
        x_fit = sol.x
    else:
        # 3段階グリッド探索
        def grid_best(xc, widths, grids):
            ...
            for w, n in zip(widths, grids):
                ...
                for idx in np.ndindex(*[n]*len(xc)):
                    ...
                    th = theta_from_L_cm(L_model(p_meas, L0_cm, x, corr_fn, replace=replace_flag), R_m, r_m, L_off_m)
                    rmse = np.sqrt(np.mean((th - theta_meas)**2))
                    if rmse < best_rmse:
                        best_rmse, best_x = float(rmse), x.copy()
                xc = best_x
            return best_x
        x_fit = grid_best(x0, widths=[(ub-lb)*0.8, (ub-lb)*0.3, (ub-lb)*0.1], grids=[11,11,13])

    # 評価
    L_fit = L_model(p_meas, L0_cm, x_fit, corr_fn, replace=replace_flag)
    theta_fit = theta_from_L_cm(L_fit, R_m, r_m, L_off_m)
    rmse, mae, bias = metrics(theta_fit, theta_meas)

    # 連続曲線
    p_dense = np.linspace(p_min_bar, p_max_bar, num_points)
    L_dense = L_model(p_dense, L0_cm, x_fit, corr_fn, replace=replace_flag)
    theta_dense = theta_from_L_cm(L_dense, R_m, r_m, L_off_m)
    ...
    return {
        ...
        "p_dense": p_dense, "L_dense": L_dense, "theta_dense": theta_dense
    }

# ===== 一括フィット =====
results = []
for mode in PLOT_MODES:
    for m in ["power", "exp_sat", "logistic", "sat_power"]:
        res = fit_model(m, mode)
        results.append(res)
        print(f"\n=== Fitted: {m} (mode={mode}) ===")
        print("params =", ", ".join([f"{v:.6f}" for v in res["x_fit"]]))
        print(f"RMSE={res['rmse']:.6f} rad, MAE={res['mae']:.6f} rad, Bias={res['bias']:.6f} rad")
...
for mode in PLOT_MODES:
    ...
    for res in results:
        if res["mode"] != mode: continue
        label = f"L new ({res['model']}, {mode})"
        plt.plot(res["p_dense"], res["L_dense"], label=label)
    plt.xlabel("Pressure p [bar]"); plt.ylabel("L [cm]")
    plt.title(f"L vs p: base vs all corrections (mode={mode})")
    ...
    for res in results:
        if res["mode"] != mode: continue
        label = f"theta new ({res['model']}, {mode})"
        plt.plot(res["p_dense"], res["theta_dense"], label=label)
    plt.plot(p_meas, theta_meas, "o", label="Experiment")
    plt.xlabel("Pressure p [bar]"); plt.ylabel("theta [rad]")
    plt.title(f"theta vs p: base vs all corrections vs experiment (mode={mode})")
    plt.grid(True); plt.xlim(*P_RANGE); plt.ylim(*THETA_RANGE); plt.legend(ncol=1); plt.tight_layout()
