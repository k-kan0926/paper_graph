# model_exp_fit_corr_joint.py
# ---------------------------------------------------------
# 目的: θ(p) 実験値に合わせて、L(p) コアの「アフィン補正 (S, A[cm])」と
#      新・飽和型補正（推奨: sat_power）、さらに L_off を同時同定。
# 特徴:
#  - 旧補正 (-0.009*L0*sqrt(p)) は replace=True なら除去、False なら残す
#  - SciPy least_squares（soft_l1）でフィット（無ければ簡易グリッドにフォールバック）
#  - L'(p) ≤ 0 の単調性ペナルティを追加（暴れ防止）
#
# 実行後:
#  - ベースラインとフィット結果の RMSE/MAE/Bias を出力
#  - L(p), θ(p) の前後比較を描画
...
INCLUDE_CORR   = True    # 新しい補正項 ΔL を使う
FIT_L_OFF      = False    # L_off も同定する

# ===== 補正モデルの選択と振る舞い =====
# 'power'    : ΔL =  K * L0 * p^α
# 'exp_sat'  : ΔL =  K * L0 * (1 - exp(-λ p))
# 'logistic' : ΔL =  S * L0 / (1 + exp(-(p - p0)/w))
# 'sat_power': ΔL =  K * L0 * (p^α) / (1 + (p/ps)^α)   ←推奨
MODEL_NAME = "sat_power"

# 既存の -0.009 L0 sqrt(p) を置き換えるか？
REPLACE_BASE_CORR = True  # True=除去, False=残す（その上にΔLを加える）

# K の符号を許容（旧補正を残す場合、K<0で相殺も可能）
ALLOW_SIGNED_K = True
...
def build_param_spec():
    ...
    if INCLUDE_AFFINE:
        names += ["S_core","A_cm"]
        x0 += [1.0, 0.0]          # 初期: 無補正
        lb += [0.85, -2.0]        # スケール±15%, シフト±2 cm
        ub += [1.15,  2.0]
    # 2) correction params
    if INCLUDE_CORR:
        corr, npar = corr_fn(MODEL_NAME)
        if MODEL_NAME == "power":
            names += ["K","alpha"]
            x0 += [0.00, 0.8]
...
def unpack_params(names, x):
    ...
    return S_core, A_cm, np.array(pvec), L_off

def L_model_from_params(p, L0_cm, names, x, replace=True):
    S_core, A_cm, corr_params, _ = unpack_params(names, x)
...
# ---- ベースライン（旧√p補正を含めるかは REPLACE_BASE_CORR に従う）----
...
# ---- フィット（SciPy → フォールバック）----
names, x0, lb, ub = build_param_spec()
...
def residual_with_penalty(x):
    # データ残差
    L_cm = L_model_from_params(p_meas, L0_cm, names, x, replace=REPLACE_BASE_CORR)
    S_core, A_cm, corr_params, L_off_m = unpack_params(names, x)
    th   = theta_from_L_cm(L_cm, R_m, r_m, L_off_m)
    res  = th - theta_meas
    # 単調性ペナルティ（L'(p) <= 0 を好む）
    L_dense = L_model_from_params(p_dense, L0_cm, names, x, replace=REPLACE_BASE_CORR)
...
if use_scipy:
    sol = least_squares(residual_with_penalty, x0, bounds=(lb, ub), loss='soft_l1', f_scale=0.05, max_nfev=5000)
    x_fit = sol.x
else:
    # パラメータが増えるためフォールバックは簡易：x0 周りを各次元ごとに少しずつ探索
    x_fit = x0.copy()
    for it in range(3):  # 3周だけ粗い調整
        for i in range(len(x0)):
            ...
            for val in grid:
                ...
                L_cm  = L_model_from_params(p_meas, L0_cm, names, x_try, replace=REPLACE_BASE_CORR)
                _, _, _, L_off_m = unpack_params(names, x_try)
                th = theta_from_L_cm(L_cm, R_m, r_m, L_off_m)
...
# ---- 結果評価 ----
L_cm_fit = L_model_from_params(p_meas, L0_cm, names, x_fit, replace=REPLACE_BASE_CORR)
S_core, A_cm, corr_params, L_off_fit = unpack_params(names, x_fit)
theta_fit = theta_from_L_cm(L_cm_fit, R_m, r_m, L_off_fit)
rmse1, mae1, bias1, res1 = metrics(theta_fit, theta_meas)

print("\n=== Fitted params ===")
for k, v in zip(names, x_fit):
    print(f"{k} = {v:.6f}")
...
L_fit_plot = L_model_from_params(p_plot, L0_cm, names, x_fit, replace=REPLACE_BASE_CORR)
theta_fit_plot = theta_from_L_cm(L_fit_plot, R_m, r_m, L_off_fit)
