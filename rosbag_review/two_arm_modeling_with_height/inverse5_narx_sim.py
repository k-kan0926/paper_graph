...
# -*- coding: utf-8 -*-
"""
NARX共通NPZを用いた簡易逆写像シミュレーション（収束可視化付き）
- NARX: theta_next = f([theta_{t-1..t-lag_y}], [ps/pd]_{t-1-delay..t-lag_u-delay})
- 制御: receding式（1変数ステップ）で (ps_t,pd_t) を選び、delay+1 ステップ先の誤差を最小化
- ソルバー: SciPy不要の粗密グリッド探索（L-BFGS-B等が使える環境なら置換可）

使い方:
  python inverse5_narx_sim.py \
  ...
  --target 20 \
  --T 300 \
  --w-theta 1.0 --w-z 0.2 --w-rate 0.05 \
  --p-lo 0.0 --p-hi 1.0
"""
...
def build_poly_from_vector(x_vec, names):
    x = {f'x{i}': x_vec[i] for i in range(len(x_vec))}
    feats = []
    for nm in names:
        if nm == '1':
            feats.append(1.0)
...
def main():
    ...
    meta["coef"] = np.asarray(meta["coef"], float)
    meta["poly_feature_names"] = [str(s) for s in meta["poly_feature_names"]]
    meta["z_coef"] = np.asarray(meta["z_coef"], float)
    meta["z_feat_names"] = [str(s) for s in meta["z_feat_names"]]
    ...
    plt.title("Inverse simulation with NARX (receding, coarse search)")
