...
# -*- coding: utf-8 -*-
"""
Train a multi-output NARX-like MLP for antagonistic McKibben system from CSV sessions (p1/p2 version).
Outputs: [theta_{t+1}[rad], dz_{t+1}[m]]

Required CSV columns:
  t[s], p1[MPa], p2[MPa], p1_cmd[MPa], p2_cmd[MPa],
  p_sum[MPa], p_diff[MPa],   # ← あっても無視（互換のため残しOK）
...
Example:
  python fit6_3_train_narx_from_csvs_z_p1p2.py \
    ...
    --out_dir out_narx/out_narx_p1p2 --lags 12 --delay 2 --epochs 250 \
...
REQUIRED_COLS = [
    "t[s]","p1[MPa]","p2[MPa]","p1_cmd[MPa]","p2_cmd[MPa]",
    "theta[rad]","theta[deg]","z[m]","dz[m]"
    # p_sum/p_diff は不要だが CSV に残っていても可
]
...
def load_csv(path: str):
    ...
    # Derivatives of p1/p2 (features)
    for col, dcol in [("p1[MPa]","dp1[MPa/s]"), ("p2[MPa]","dp2[MPa/s]")]:
        x = df[col].to_numpy()
        dx = np.zeros_like(x)
        if len(x) > 1:
            dx[1:] = (x[1:] - x[:-1]) / dt
...
# ---------------------- Static dz surrogate fit: dz ≈ quad(p1,p2) ----------------------
def fit_fz_quadratic_from_static_p1p2(stat_csvs, ss_theta_eps, ss_z_eps):
    """
    Collect quasi-static samples from static CSVs and fit:
      dz ≈ a0 + a1*p1 + a2*p2 + a3*p1^2 + a4*p2^2 + a5*p1*p2
    Returns dict with coefficients and diagnostics, or None if insufficient samples.
    """
    p1_list, p2_list, dz_list = [], [], []
    for spath in stat_csvs:
        ...
        if len(qs) == 0:
            continue
        p1_list.append(qs["p1[MPa]"].to_numpy())
        p2_list.append(qs["p2[MPa]"].to_numpy())
        dz_list.append(qs["dz[m]"].to_numpy())

    if not p1_list:
        return None

    p1 = np.concatenate(p1_list).astype(np.float64)
    p2 = np.concatenate(p2_list).astype(np.float64)
    dz = np.concatenate(dz_list).astype(np.float64)

    # remove NaN/Inf
    ok = np.isfinite(p1) & np.isfinite(p2) & np.isfinite(dz)
    p1, p2, dz = p1[ok], p2[ok], dz[ok]
    if len(p1) < 30:
        return None

    # robust-ish trimming to suppress outliers (2–98 percentile on each)
    p1_lo, p1_hi = np.percentile(p1, [2.0, 98.0])
    p2_lo, p2_hi = np.percentile(p2, [2.0, 98.0])
    keep = (p1 >= p1_lo) & (p1 <= p1_hi) & (p2 >= p2_lo) & (p2 <= p2_hi)
    if np.sum(keep) >= 30:
        p1_t, p2_t, dz_t = p1[keep], p2[keep], dz[keep]
    else:
        p1_t, p2_t, dz_t = p1, p2, dz

    # design matrix for quadratic terms
    Phi = np.stack([
        np.ones_like(p1_t),
        p1_t, p2_t,
        p1_t**2, p2_t**2,
    ...
    # least squares
    coeffs, *_ = np.linalg.lstsq(Phi, dz_t, rcond=None)  # a0..a5
    a0,a1,a2,a3,a4,a5 = map(float, coeffs)
    ...
    return {
        ...
        "p1_min": float(np.min(p1)), "p1_max": float(np.max(p1)),
        "p2_min": float(np.min(p2)), "p2_max": float(np.max(p2)),
        "n": int(len(p1)),
        "r2": r2,
        "basis": "dz ≈ a0 + a1*p1 + a2*p2 + a3*p1^2 + a4*p2^2 + a5*p1*p2"
    }
...
def train(args):
    ...
    # ---- fit dz surrogate f_z(p1,p2) on quasi-static samples from stat CSVs (optional)
    fz_meta = None
    if args.stat_csvs:
        try:
            fz_meta = fit_fz_quadratic_from_static_p1p2(args.stat_csvs, args.ss_theta_eps, args.ss_z_eps)
        except Exception as e:
            print(f"[warn] f_z(p1,p2) fit failed: {e}")
            fz_meta = None
    ...
    meta = {
        ...
        "theta_train_minmax": list(theta_range_from_csvs([p for p,_,_ in tr_slices])) if tr_slices else [-math.pi, math.pi],
        "dz_train_minmax": list(dz_range_from_csvs([p for p,_,_ in tr_slices])) if tr_slices else [-0.1, 0.1],
        "dt_est": float(dt_est),
        "pressure_limits": {
            "p_max_each_side_MPa": args.p_max_each_side_MPa,   # optional
            "p1_rate_limit_MPa_s": args.p1_rate_limit_MPa_s,   # optional
            "p2_rate_limit_MPa_s": args.p2_rate_limit_MPa_s    # optional
        },
        # ---- dz surrogate coefficients (if available)
        "fz_p1p2_quadratic": fz_meta  # dict or None
    }
    os.makedirs(args.out_dir, exist_ok=True)
    ...
    if fz_meta is not None:
        print("dz surrogate f_z(p1,p2): " +
              ", ".join([f"{k}={fz_meta[k]:.6g}" for k in ["a0","a1","a2","a3","a4","a5"]]) +
              f", R2={fz_meta['r2']:.3f}, n={fz_meta['n']}, "
              f"p1∈[{fz_meta['p1_min']:.3f},{fz_meta['p1_max']:.3f}], "
              f"p2∈[{fz_meta['p2_min']:.3f},{fz_meta['p2_max']:.3f}]")
    else:
        print("dz surrogate f_z(p1,p2): not available (insufficient or invalid static data).")
...
def parse_args():
    ...
    ap.add_argument("--p1_rate_limit_MPa_s", type=float, default=None,
                    help="Optional: |dp1/dt| limit [MPa/s] to record into meta")
    ap.add_argument("--p2_rate_limit_MPa_s", type=float, default=None,
                    help="Optional: |dp2/dt| limit [MPa/s] to record into meta")
