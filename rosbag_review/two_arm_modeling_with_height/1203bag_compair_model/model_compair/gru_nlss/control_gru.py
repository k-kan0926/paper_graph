...
class GRU_NLSS(nn.Module):
    ...
    def forward(self, u_seq, h0=None):
        h_seq, h_final = self.gru(u_seq, h0)
...
class BaseGRUController:
    def __init__(self, model_dir, dt=0.01, device='cpu'):
        ...
        # Hidden state
        self.h = self.model.init_hidden(1, device)
    ...
    def load_model(self, model_dir, device):
        with open(os.path.join(model_dir, 'gru_nlss_meta.json'), 'r') as f:
        ...
        self.model.load_state_dict(
            torch.load(os.path.join(model_dir, 'gru_nlss_model.pt'),
                      map_location=device)
        )
        self.model.to(device)
        self.model.eval()
        
        print(f"[Model] Loaded GRU-NLSS: hidden={self.hidden_dim}, layers={self.num_layers}")
    ...
    def predict_step(self, p1, p2, h=None):
        """1ステップ予測"""
        ...
        with torch.no_grad():
            theta, h_new = self.model(u, h)
        
        theta_val = float(theta[0, 0, 0].cpu().numpy())
        
        return theta_val, h_new
    ...
    def step(self, theta_ref):
        ...
        # Predict next state
        theta_next, h_new = self.predict_step(p1_cmd, p2_cmd, self.h)
...
# ==================== 1. MPPI Controller ====================
class GRU_MPPI(BaseGRUController):
    def __init__(self, model_dir, dt=0.01, device='cpu',
    ...
    def compute_control(self, theta_ref):
        ...
        for k in range(self.K):
            ...
            for h_idx in range(self.H):
                ...
                # Predict
                theta, h = self.predict_step(p1, p2, h)
...
class GRU_GradientMPC(BaseGRUController):
    """勾配降下でMPC"""
    ...
    def compute_control(self, theta_ref):
        ...
        for _ in range(self.n_iter):
            ...
            # Forward pass
            theta_seq, _ = self.model(u_seq, self.h)  # (1, H, 1)
...
# ==================== 3. Random Shooting ====================
class GRU_RandomShooting(BaseGRUController):
    def __init__(self, model_dir, dt=0.01, device='cpu',
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='gru_nlss_control_results')
    ...
    print(f"\n{'='*70}")
    print(f" GRU-NLSS Control Comparison")
    print(f"{'='*70}")
    ...
    for ctrl_name in args.controllers:
        ...
        if ctrl_name == 'mppi':
            ctrl = GRU_MPPI(args.model_dir, args.dt, device, K=32, H=15)
        elif ctrl_name == 'gradient_mpc':
            ctrl = GRU_GradientMPC(args.model_dir, args.dt, device, horizon=10, n_iter=50)
        elif ctrl_name == 'random_shooting':
            ctrl = GRU_RandomShooting(args.model_dir, args.dt, device, K=64, H=15)
