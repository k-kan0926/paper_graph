...
"""
train_lstm_ss.py
LSTM-based State Space Model (LSTM-SS)の訓練

構造:
- 潜在状態: (h, c) ∈ R^n × R^n (hidden & cell state)
- 状態遷移: (h_{t+1}, c_{t+1}) = LSTM((h_t, c_t), u_t)
- 観測方程式: θ_t = g(h_t)  (MLPデコーダ)

LSTMの特徴:
- GRUより複雑（cell state + hidden state）
- 長期記憶能力が高い
- より表現力が高いが計算コストも高い
"""
...
class LSTM_SS(nn.Module):
    ...
    def __init__(self, input_dim=2, hidden_dim=64, num_layers=2):
        ...
        # State transition: (h_{t+1}, c_{t+1}) = LSTM((h_t, c_t), u_t)
        self.lstm = nn.LSTM(
        ...
        print(f"[LSTM-SS] Input: {input_dim}, Hidden: {hidden_dim}, Layers: {num_layers}")
    
    def forward(self, u_seq, h0=None, c0=None):
        """
        Args:
            u_seq: (batch, seq_len, input_dim) - 制御入力シーケンス
            h0: (num_layers, batch, hidden_dim) - 初期hidden state（オプション）
            c0: (num_layers, batch, hidden_dim) - 初期cell state（オプション）
        
        Returns:
            theta: (batch, seq_len, 1) - 予測角度シーケンス
            h_final: (num_layers, batch, hidden_dim) - 最終hidden state
            c_final: (num_layers, batch, hidden_dim) - 最終cell state
        """
        # LSTM forward
...
def train_model(model, train_loader, val_loader, device, epochs=300, lr=1e-3, patience=30):
    ...
    for epoch in range(epochs):
        ...
        for U_batch, Theta_batch in train_loader:
            ...
            theta_pred, _, _ = model(U_batch, h0, c0)
...
def main():
    ...
    parser.add_argument('--out_dir', type=str, default='lstm_ss_model')
    ...
    print(f"\n{'='*70}")
    print(f" LSTM State Space Model Training")
    print(f"{'='*70}")
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, 'lstm_ss_model.pt'))
    ...
    with open(os.path.join(args.out_dir, 'lstm_ss_meta.json'), 'w') as f:
    ...
    print(f"\n[Saved] {args.out_dir}/")
    print(f"  - lstm_ss_model.pt")
    print(f"  - lstm_ss_meta.json")
    ...
    print(f"  - Better long-term memory (cell state)")
    print(f"  - More expressive (separate forget/input gates)")
    print(f"")
    print(f"LSTM disadvantages:")
    print(f"  - More parameters → slower training")
    print(f"  - More complex → harder to tune")
