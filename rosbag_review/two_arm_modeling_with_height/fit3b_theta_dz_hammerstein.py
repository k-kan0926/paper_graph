...
- column standardization for features,
- model selection by rollout RMSE.

Model:
  dθ/dt = α*(θ_stat(Σ,Δ) - θ) + kΣ*Σ̇ + kΔ*Δ̇   using inputs at (t - d*dt)

Usage:
  python fit3b_theta_dz_hammerstein.py --csv out/diff_run1_h_data.csv --mono-model out/model_k2/model_k2_mono_model.npz --out-prefix out/model_k3b/model_k3b --delay-grid 0,1,2,3 --l2 5e-3 --sg-win 17 --kdelta-min -2.0
"""
...
def build_features(ps_s, pdiff_s, th_s, dps, dpdiff, d, dth, a,b,c, dt):
    ...
    We need (k-d-1) >= 0 for rate terms -> k starts from d+1.
    ...
    ks  = idx
    ku  = idx - d          # for θ_stat(Σ,Δ) and rates
    kup = idx - d - 1      # previous for rates

    phi1 = theta_static(ps_s[ku], pdiff_s[ku], a,b,c) - th_s[ks]
    dS   = ps_s[ku]   - ps_s[kup]
    dD   = pdiff_s[ku]- pdiff_s[kup]
    y    = dth[ks]

    X = np.column_stack([phi1, dS/dt, dD/dt])  # units: [deg], [MPa/s], [MPa/s] -> y is [deg/s]
    return X, y, start
