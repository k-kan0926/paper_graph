...
class NARX_MPPI_Simulator:
    ...
    def run_simulation(self, theta_target_rad, steps, smooth_ref=True):
        """シミュレーション実行"""
        ...
        for step in range(steps):
            ...
            # Log
            error = theta_target_rad - self.theta_rad
            self.log_time.append(t)
            self.log_theta.append(self.theta_rad)
            self.log_theta_ref.append(theta_ref_traj[0])
            ...
            if step % 10 == 0:
                print(f"[{step:03d}] t={t:.2f}s, theta={math.degrees(self.theta_rad):.2f}°, "
                      f"ref={math.degrees(theta_ref_traj[0]):.2f}°, "
