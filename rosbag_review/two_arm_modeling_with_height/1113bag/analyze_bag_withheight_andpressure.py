...
# Check pandas import explicitly
try:
    import pandas as _pd
    # Verify DataFrame is accessible
    _ = _pd.DataFrame
except (ImportError, AttributeError) as e:
    print(f"[ERROR] pandas not properly imported: {e}")
...
def main():
    ...
    ap.add_argument("--height-topic", default="/arm3/pose")
    ...
    if len(t_p1_cmd) > 0:
        p1_cmd = nearest_interp(t_ref, t_p1_cmd, v_p1_cmd)
    else:
        p1_cmd = nearest_interp(t_ref, t_p1_cmd, v_p1_cmd)  # 実際はここに来ない想定

    # p2_cmd は必ず t_ref に補間してそろえる
    if len(t_p2_cmd) > 0:
        p2_cmd = nearest_interp(t_ref, t_p2_cmd, v_p2_cmd)
    else:
        p2_cmd = np.full_like(ts, np.nan)
    ...
    dt_median = np.median(np.diff(ts)) if len(ts) > 1 else 0.005
    
    def gradient_safe(x, dt_val):
    ...
    dps_dt = gradient_safe(ps, dt_median)
    dpd_dt = gradient_safe(pd, dt_median)
    
    # Build DataFrame - ensure pandas is imported correctly
    try:
        # Create DataFrame with explicit module reference
        df = _pd.DataFrame({
            "t[s]": ts,
            "p1_cmd[MPa]": p1_cmd,
            "p2_cmd[MPa]": p2_cmd,
        ...
        print("[DEBUG] pandas version:", pd.__version__ if hasattr(pd, '__version__') else "unknown")
        print("[DEBUG] pandas module:", pd)
        print("[DEBUG] DataFrame attribute:", hasattr(pd, 'DataFrame'))
        raise
    ...
    print(f"Sampling rate: {1.0/dt_median:.1f} Hz (median dt = {dt_median*1000:.2f} ms)")
