...
# -*- coding: utf-8 -*-
"""
Phase-1: Model zoo benchmark (+ z(ps,pd) learning/design, progress logs, vectorized static eval)
...
Outputs:
  ...
  out-dir/{hw_poly|hw_rbf|pwa|gp|mononn}_common_meta.npz  <-- MPCなどで読みやすい共通メタ
  out-dir/narx_meta.json
...
def fit_z_learn(ps, pd, z_obs, feat_mode="quad", lam=1e-6, val_ratio=0.2):
    """
    z = Phi(ps,pd) w を学習し、検証RMSE等のメトリクスを返す。
    戻り: (coef, feat_names, metrics_dict)
      metrics_dict: {rmse_tr, mae_tr, r2_tr, rmse_va, mae_va, r2_va}
    """
...
def fit_or_design_z(ps, pd, z_obs=None, feat_mode="quad",
    ...
    """
    戻り: (coef, feat_names, metrics or None)
    """
    if z_obs is None:
        Phi, names = _build_z_features(ps, pd, feat_mode)
...
def save_common_meta_npz(out_path_npz, dt, muS, sdS, muD, sdD, dyn, z_coef, z_feat_names, z_metrics=None):
    """
    逆写像/MPC 側で使いやすい共通メタを npz として保存。
    dyn: dict(alpha,kS,kD,d,rmse)
    z_metrics: dict or None -> z_rmse_va / z_mae_va / z_r2_va を書き込み
    """
...
def main():
    ...
    try:
        ...
        if args.z_mode == "learn":
            if args.z_col is None or args.z_col not in df.columns:
                raise RuntimeError("--z-mode learn には --z-col=<列名> が必要です")
            z_obs = df[args.z_col].to_numpy(float)
        ...
        # 便利ヘルパ：各 static+dyn 後に z を推定/設計し、JSON と NPZ を保存
        def finalize_static_model(tag_name, pack_dict):
            # z 係数 + メトリクス取得
            z_coef, z_feat_names, z_metrics = fit_or_design_z(
                ps, pdv,
                z_obs=(z_obs if args.z_mode=="learn" else None),
                feat_mode=args.z_feat,
        ...
        if flags["narx"] and SKLEARN_OK:
            ...
            best_meta=None
            for d in [int(x) for x in args.delay_grid.split(",") if x.strip()!=""]:
                rmse, meta = narx_fit_predict(ps, pdv, th,
            ...
            # JSON（列名も保存）
            narx_json = dict(best_meta)
            save_json(os.path.join(args.out_dir, "narx_meta.json"), narx_json)

            # ---- z: 学習 or 設計 → 係数と名前 + メトリクス取得
            z_coef, z_feat_names, z_metrics = fit_or_design_z(
                ps, pdv,
                z_obs=(z_obs if args.z_mode=="learn" else None),
                feat_mode=args.z_feat,
            ...
            # JSON 追記
            narx_json["z_coef"] = np.asarray(z_coef, float).tolist()
            narx_json["z_feat_names"] = z_feat_names
            narx_json["z_mode"] = args.z_mode
            narx_json["z_feat"] = args.z_feat
            if z_metrics is not None:
                narx_json["z_metrics"] = z_metrics
            save_json(os.path.join(args.out_dir, "narx_meta.json"), narx_json)

            # ---- NARX 共通 npz（θ=NARX + z の係数を同梱）
            narx_npz = os.path.join(args.out_dir, "narx_common_meta.npz")
            theta_unit = "deg"  # 本スクリプトの列は deg 前提
